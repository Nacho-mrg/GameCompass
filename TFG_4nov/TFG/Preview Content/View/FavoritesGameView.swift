...
struct FavoriteGamesView: View {
    ...
    private func refreshGames(for favoriteIDs: [Int]) async {
        ...
        do {
            let games = try await loadGamesViaSharedAPIPlaceholderMock(ids: intIDs)
            await MainActor.run {
    ...
    // MARK: - Placeholder Mock (Reemplazar con tu lógica de API real)
    private func loadGamesViaSharedAPIPlaceholderMock(ids: [Int]) async throws -> [Game] {
        return ids.map { id in
            let imageID = abs(id) % 1000
            return Game(
                id: id,
                name: "Juego #\(id)",
                backgroundImage: "https://picsum.photos/id/\(imageID)/800/400"
            )
        }
    }
}
